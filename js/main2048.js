	...
	for(var i=0; i<4; i++){
		for(var j=0; j<4; j++){
			...
			if(board[i][j] == 0){
				...
				theNumberCell.css({
					'background-image': `url(${getTileImage(board[i][j])})`,
...
function moveLeft(){
	...
	setTimeout(() => {
      updateBoardView();
      setTimeout(() => isGameOver(), 50); // 确保视图刷新后再判断
    }, 200);
...
// 保存分数到排行榜
function saveToLeaderboard1(name, score) {
  $.post("http://你的域名/2048/leaderboard.php?action=add", {
    name: name,
    score: score
  }, function(res) {
...
function renderLeaderboard() {
  $.get("http://你的域名/2048/leaderboard.php?action=get", function(data) {
